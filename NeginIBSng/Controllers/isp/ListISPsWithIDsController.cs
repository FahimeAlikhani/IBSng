...
namespace NeginIBSng.Controllers.isp
    {
        ...
        [ApiController]
        public class ListISPsWithIDsController : ControllerBase
        {
            private readonly IConfiguration _configuration;
            public ListISPsWithIDsController(IConfiguration configuration)
            {
                _configuration = configuration;
            }
            public async Task<IActionResult> Post(SendModel<ListISPsWithIDsInputModel> model)
            {
                var result = new ApiModol<ListISPsWithIDsOutputModel>();
                ...
                var replyModel = Newtonsoft.Json.JsonConvert.DeserializeObject<ListISPsWithIDsOutputModel>(stringJson);
