...
namespace NeginIBSng.Controllers.isp
    {
        ...
        [ApiController]
        public class DeleteISPController : ControllerBase
        {
            private readonly IConfiguration _configuration;
            public DeleteISPController(IConfiguration configuration)
            {
                _configuration = configuration;
            }
            public async Task<IActionResult> Post(SendModel<DeleteISPInputModel> model)
            {
                var result = new ApiModol<DeleteISPOutputModel>();
                ...
                var replyModel = Newtonsoft.Json.JsonConvert.DeserializeObject<DeleteISPOutputModel>(stringJson);
